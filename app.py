...
# ── ONESIGNAL PUSH ─────────────────────────────────────────────────────
# Matches the working pattern from the reference implementation:
# "Authorization": "Basic " + key  +  requests.post  +  web_url field
...
def push(title, message, severity="medium", url="/dashboard"):
    ...
    payload = {
        ...
        "headings":          {"en": full_title},
        "contents":          {"en": message},
        "web_url":           url,          # web_url (not "url") per working reference
    ...
    try:
        resp = _req.post(ONESIGNAL_URL, headers=_os_headers(), json=payload, timeout=10)
        result = resp.json()
        if resp.status_code == 200:
            print(f"[Push OK] {result.get('recipients','?')} recipients — {full_title}")
        else:
            print(f"[Push FAIL] {resp.status_code}: {result}")
